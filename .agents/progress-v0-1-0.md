...
Current status:
- Milestone 10 implemented and verified with offline and live tests.

Next milestone to implement:
- None. v0.1.0 milestones are complete.
...
- Added a gated `_should_run_live_tests()` test helper, a small real-network FRED integration test, and an opt-in manual GitHub Actions live-test job in milestone 10.
- Live tests require both `ENV["FRED_API_KEY"]` and `ENV["RUN_LIVE_TESTS"] == "true"`; `.env` alone does not enable the live test gate.
